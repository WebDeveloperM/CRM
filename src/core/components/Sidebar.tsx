        ...
        <div className={`h-full bg-white fixed top-[57px] z-20 left-0  ${open ? "w-64" : "md:w-20 w-0"}  duration-300 `}>
            ...
            <div className="h-full relative shadow">
                ...
                <ul className="pt-1 scrollbar-thumb-indigo-50 scrollbar-white h-2/3 overflow-y-scroll">
                    <Link to="/dashboard" className={`flex items-center   pl-[18px] text-secondary text-sm gap-x-4 cursor-pointer px-2 py-1.5 hover:bg-secondary/10 rounded-r-full group ${pathname.startsWith("/dashboard") ? "bg-secondary-light" : ""}`}>
                        <RiHome3Line className={`text-gray-900  overflow-clip cursor-pointer w-[35px] h-[35px] p-2 rounded-lg ${pathname.startsWith("/dashboard") ? "bg-secondary text-white group-hover:bg-secondary group-hover:text-white" : "bg-secondary-light text-gray-900 group-hover:bg-white group-hover:text-secondary "}`} />
                        <span className={`origin-left text-base group-hover:text-secondary  ${pathname.startsWith("/dashboard") ? "text-secondary" : ""} duration-200 ${!open ? "scale-0" : ""} text-gray-800`}>Dashboard</span>
                    </Link>

                    <Link to="/doctors" className={`flex items-center pl-[18px] mt-0.5  group text-secondary text-sm gap-x-4 cursor-pointer px-2 py-1.5 hover:bg-secondary/10 rounded-r-full ${pathname.startsWith("/doctors") ? "bg-secondary-light" : ""}`}>
                        <FaUserDoctor className={`text-gray-900 overflow-clip cursor-pointer w-[35px] h-[35px] p-2 rounded-lg ${pathname.startsWith("/doctors") ? "bg-secondary text-white group-hover:bg-secondary group-hover:text-white" : "bg-secondary-light text-gray-900 group-hover:bg-white group-hover:text-secondary "}`} />
                        <span className={`origin-left text-base group-hover:text-secondary  ${pathname.startsWith("/doctors") ? "text-secondary" : ""} duration-200 ${!open ? "scale-0" : ""} text-gray-800`}>Doctors</span>
                    </Link>
                    ...
                    <Link to="/" onClick={() => logout()} className={`flex items-center pl-[18px] mt-0.5  group text-secondary text-sm gap-x-4 cursor-pointer px-2 py-1.5 hover:bg-secondary/10 rounded-r-full ${pathname === "/signout" ? "bg-secondary-light" : ""}`}>
